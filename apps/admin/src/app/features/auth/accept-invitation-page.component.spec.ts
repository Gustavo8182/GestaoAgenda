...
async function createComponent(token: string | null): Promise<{
  fixture: ComponentFixture<AcceptInvitationPageComponent>;
...
describe('AcceptInvitationPageComponent', () => {
  ...
  it('does not submit when the new password is too short', async () => {
    ...
    httpMock.expectNone('/api/v1/auth/invitations/accept');
    httpMock.verify();
  });
  ...
  it('submits the token and new password, then shows a success message', async () => {
    ...
    const request = httpMock.expectOne('/api/v1/auth/invitations/accept');
    expect(request.request.body).toEqual({ token: 'abc123token', newPassword: 'NovaSenhaForte456!' });
    request.flush({ message: 'Convite aceito. Você já pode entrar com a nova senha.' });
    fixture.detectChanges();
  ...
  it('shows the server error message when the token is invalid', async () => {
    ...
    const request = httpMock.expectOne('/api/v1/auth/invitations/accept');
    request.flush(
      { code: 'invalid_invitation_token', message: 'Convite inválido ou expirado.' },
      { status: 400, statusText: 'Bad Request' }
    );
    fixture.detectChanges();

    expect(fixture.nativeElement.textContent).toContain('Convite inválido ou expirado.');
    httpMock.verify();
  });
});
