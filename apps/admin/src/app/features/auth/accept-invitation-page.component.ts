...
export class AcceptInvitationPageComponent {
  ...
  protected submit(): void {
    ...
    this.authService.acceptInvitation(this.token, newPassword).subscribe({
      next: () => {
      ...
      error: (error: HttpErrorResponse) => {
        this.submitting.set(false);
        this.errorMessage.set(error.error?.message ?? 'Não foi possível aceitar o convite. Confira o link recebido por e-mail.');
      }
    });
  }
}
